...
export default function ExtrudeController({ entity }: ControllerProps) {
  const em = useEntityManager();
  const geometry = em.getComponent(Geometry, entity);
  const extrudeGeometry = geometry!.data as ExtrudeGeometryData;
  const ps = extrudeGeometry.points.map((point) => {
    const [x, y] = point;
    return new THREE.Vector2(x, y);
  });
  const shape = new THREE.Shape(ps);
