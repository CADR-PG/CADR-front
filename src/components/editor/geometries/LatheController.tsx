...
export default function LatheController({ entity }: ControllerProps) {
  const em = useEntityManager();
  const geometry = em.getComponent(Geometry, entity);
  const latheGeometry = geometry!.data as LatheGeometryData;
  const points = latheGeometry.points.map((point) => {
    const [x, y] = point;
